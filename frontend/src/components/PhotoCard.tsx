...
export const PhotoCard = ({ photo, onLike, onClickPhoto, onClickAuthor }: PhotoCardProps) => {
  ...
  return (
    <article className="flex items-stretch overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-gray-200 hover:shadow-md transition-shadow duration-150">
      {/* Thumbnail */}
      <button
        className="w-2/5 shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500"
      ...
      <div className="flex flex-col gap-2 px-4 py-3 min-w-0 flex-1">
        ...
        <div className="mt-auto flex items-center gap-1.5">
          <button
            ...
            className={`flex items-center gap-1.5 text-sm transition-colors focus-visible:outline-none ${photo.likedByMe
              ? "text-red-500"
              : "text-gray-400 hover:text-red-400"
              }`}
          >
            <i
              className={`${photo.likedByMe ? "fa-solid" : "fa-regular"
                } fa-heart`}
            />
